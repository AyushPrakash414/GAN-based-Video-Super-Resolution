...
def require_stage10_dependencies():
    try:
        import torch
    except ImportError as exc:
        raise RuntimeError(
            "Stage 10 requires PyTorch, but `torch` is not installed in the active environment.\n"
            "Install the project ML dependencies first, then resume with:\n"
            "python -m scripts.run_pipeline --start-stage 9"
        ) from exc
    ...
    try:
        ...
        raise RuntimeError(
            "Stage 10 requires `gfpgan` and its dependencies, but they are not available.\n"
            "Install the project restoration dependencies before resuming."
        ) from exc

    return torch, GFPGANer
...
def main():
    torch, GFPGANer = require_stage10_dependencies()
