...
def require_torch():
    try:
        ...
        print("Install the project ML dependencies first, then resume with:")
        print("python -m scripts.run_pipeline --start-stage 7")
...
def resolve_model_path(model_path):
    ...
    fallback_candidates = [
        candidate,
        Path("models/realesrgan/weights") / candidate.name,
    ]
...
class RealESRGANUpscaler:
    ...
    def __init__(self, model_path, scale=4, device='cuda', sharpen=True, 
        ...
        try:
            ...
            print("RealESRGAN is not installed.")
...
def process_frames(input_dir, output_dir, model_path, scale=2, device='cuda', 
    ...
    for frame_path in frame_paths:
        ...
        if not os.path.exists(output_path):
            pending_frames.append(frame_path)

    if not pending_frames:
        print("All requested frames are already present in the output directory.")
        return

    if len(pending_frames) != len(frame_paths):
        print(f"Skipping {len(frame_paths) - len(pending_frames)} frames already processed.")

    frame_paths = pending_frames
    print(f"Remaining frames to process: {len(frame_paths)}")
...
Examples:
  ...
  python scripts/stage08_superres.py --scale 2 --start_frame 0 --end_frame 10
    ...
    if not os.path.exists(resolved_model):
        ...
        print("\nPlease run: python tools/download_stage08.py")
        sys.exit(1)
